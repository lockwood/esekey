...
// +----------------------------------------------------------------------+
//
// $Id: 3/validate.php,v 1.06 2006/02/06
//
...
$personals = $db_object->getAll("SELECT t1.charge_name,
							   		t2.charge_from, 		
							   		t2.charge_amount 		
							  FROM 	charge as t1,		
								   	charge_rate as t2
...
for ($ct=0; $ct<count($personals); $ct++)
{
	if ($charge_name != $personals[$ct]['charge_name'])
	{
		$charge_name = $personals[$ct]['charge_name'];
		$i++;
	}
	$labels[$i] = $charge_name." (".$personals[$ct]['charge_amount']." per person)";
	$singles[$i] = $personals[$ct]['charge_amount'];
}
...
if ($ss == 'User') {
    if ((date("U",$vdate)/86400) - (date("U",$today)/86400) < 1) {
        $error1.='Bookings requested within 24 hours of start date can only be made by telephone.<br>';
        return;
    }
}
