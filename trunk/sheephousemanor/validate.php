...
// +----------------------------------------------------------------------+
//
// $Id: 4/validate.php,v 1.08 2006/09/03
//
...
if (!$chalet_selected) {
    $error1.='At least one cottage must be selected.<br>';
    return;
}
...
if ($ss == 'User') {
    if ((date("U",$vdate)/86400) - (date("U",mktime())/86400) < 3) {
...
// use seconds since the Unix Epoch / 86400 for reliable calculations including leap years etc.
if ((date("U",$vdate)/86400) - (date("U",mktime())/86400) < 28) {
    $deposit_rate = 1;
