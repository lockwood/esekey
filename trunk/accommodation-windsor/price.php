...
$tariff = '0'; // Assume Standard tariff and update if different
$discount = '0';
$negotiated = '';
...
// round price to £ & p even if discount = 0
...
if ($cardarray[$w] == 'Visa' || $cardarray[$w] == 'Mastercard')
{
	// 2% surcharge for credit cards
	$price = round($price*102/100, 2);
    $additional_info .= 'Includes 2% credit card fee.<br />';
}

if (($ap != null) && ($ap != '')) { // there is an agreed price, which overrides the calculated price
    $price = number_format($ap,2,'.','');
    $ap = $price;
    $tariff = '1'; // negotiated tariff
