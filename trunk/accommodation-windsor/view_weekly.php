...
while ($next_name == $c_name) {
    if (($i < 98)) { 
    	$row2 .= '<td class="awday-center" nowrap>'.$daynamearray[$datearray[$i]['day_of_week']].'<br />'.$datearray[$i]['booking_day'].'</td>';
        $calendarrows .= '<td class="awday-center" nowrap>'.$datearray[$i]['booking_day'].'</td>';
        $row0 .= '<td style="height: ¬px;"><span style="width:18px">&nbsp;</span></td>';
    }
    $i++;
	$next_name = '';
	if ($datearray[$i]['property_number'] != ''){
	...
	$next_name .= $datearray[$i]['property_name'];
    if (($datearray[$i]['booking_month'] == $c_month) && ($next_name == $c_name)) { 
        $monthcol++;
    } else
    {
        $row1 .= '<td class="year" colspan="'.$monthcol.'"><span style="width:'.(($monthcol*20)).'px;overflow:hidden;text-align:center;">'.$montharray[$c_month+0].'</span></td>';
...
while ($i < count($datearray)) {
	$propertyrows .= '
	  <tr>
    	<td class="property_top" nowrap>'.$c_name.'</td>
	  </tr>';
	$next_name = '';
	if ($datearray[$i]['property_number'] != ''){
		$next_name = $datearray[$i]['property_number']." ";
        	...
        	if ($datearray[$i]['display_status'] != $b_stat) {
        		...
        		if ($b_cols > 0) {
            		if (($datearray[$i-1]['display_status'] != 'E') && ($ss == 'Admin')) {// hyperlink to booking ref
            			...
            			$bookings[$row] .= 'colspan="'.$b_cols.'" onMouseOver="doHover(this);return window.status=&#039;EseBooking&trade; &amp;gt; Selected Booking&#039;;" onMouseOut="noHover(this);return window.status=&#039;&#039;;" onClick="top.Top.GoToURL(&#039;1&#039;, &#039;Edit Booking [Ref='.$datearray[$i-1]['booking_reference'].']&#039;, &#039;edit.php?view=booking_view&key=booking_reference&value='.$datearray[$i-1]['booking_reference'].'&&#039;);" nowrap><span style="width:'.(($b_cols*20)).'px;overflow:hidden;">'.$datearray[$i-1]['booking_reference'].'&nbsp;'.$tenant_name.'</span></td>';
            		} else {
...
?>
  ...
  <td>
  <div id="scrollmaster">
    <table border="0" style="margin-top:-17px;"><?=html_entity_decode($calendarrows)?>
	</table>
<div style="position:absolute;left:0px;top:0px;z-index:-1">
<table border="1">
  <?=str_replace('¬', $overlay_height, $row0)?>
</table>
</div>        
    </div>
    </td>
