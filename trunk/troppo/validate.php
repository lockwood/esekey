...
if (isset($o[2])) { // rowing special offer - $rowing_rate
    ...
    if (($guests_remaining < 8) && ($ss == 'User')) { 
    	// offer only valid for 8 or more people, can be overridden by Admin
    ...
    $a_ct = 0;
   	...
   	if ($guests_remaining < 10) {
    	...
    	if ($a_ct < 3) { 
    		// For bookings of over 9 rowers, all three apartments must be booked
        	$error1.='For bookings of over 9 rowers, all three apartments must be booked.<br>';
        	return;
    	}
...
} else 
{
    ...
    if (isset($r[1])) { // Wisteria
        ...
        if ($guests_remaining > 3) {
            if ((!isset($r[3])) && (!isset($r[2]))) { //all remaining adults in Wisteria
...
if ($ss == 'User') {
    ...
    if (($end_date > '2014-12-19' ) && ($start_date < '2015-01-03' )) {
    ...
    /*
    if (($end_date > '2012-07-26' ) && ($start_date < '2012-09-10' )) {
           $error1.='Bookings for the Olympics (26th July - 10th September 2012) can only be arranged by emailing us on <a href="mailto:enquiries@troppo.uk.com">enquiries@troppo.uk.com</a>.  We wish to avoid short bookings.  Thanks!<br>';
        return;
    }
	// */
    /* removed because of change of plans  03/04/2008
    if ((!isset($r[1])) && (!isset($r[2]))) {
	    $end_date = $db_object->getOne("SELECT date_add('".$start_date."', interval '".$n."' day)");
	    if (($end_date < '2008-10-01' ) && ($start_date > '2008-05-31' )) {
            $error1.='During the period 1st June to 30th September 2008, The Smithy can only be booked in conjunction with Wisteria or Gardener&#039;s Bothy.<br>';
	        return;
	    }
    }
    //*/
