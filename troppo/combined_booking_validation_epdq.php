...
//if (isset($_GET['res'])) { // return from Barclaycard payment gateway.....	if ($_GET['res'] == '1' || ($ss == 'Admin')) {		$update_pending_booking = $db_object->query("UPDATE booking                                                  SET display_status = 'P',													 booking_status = 'Deposit Due',                                                     last_modified_on = now()                                               WHERE company_id = '".$_SESSION[$ss]['company_id']."'                                                 AND booking_reference = '".$_SESSION[$ss]['booking_reference']."';");		if (DB::isError($update_pending_booking)) {    		die($update_pending_booking->getMessage());		}		if (!isset($_SESSION[$ss]['sent']) && isset($_SESSION[$ss]['booking_reference'])) {			include($DOCUMENT_ROOT.'/'.$_SESSION[$ss]['company_code'].'/email.php');			$_SESSION[$ss]['sent'] = 1;		}	}	return;}
if (isset($_GET['conditions'])) { // booking conditions accepted - this is a new booking - ensure booking reference null
    unset($_SESSION[$ss]['booking_reference']);    unset($_SESSION[$ss]['sent']);    
}
