<?php
// +----------------------------------------------------------------------+
// | IDXS  - the secure EseSite runtime driver - Company 3                |
// +----------------------------------------------------------------------+
...
// +----------------------------------------------------------------------+
//
// $Id: 3/idxs.php,v 1.01 2005/04/11
//
...
$window_type = 'p';
if ($_GET['p'] != 13) { 
    // secure booking  page 13 only - otherwise redirect to home
...
if ((!isset($_GET['conditions'])) && (!isset($_GET['d'])) && (!isset($_GET['pgt']))) { // booking conditions not accepted or form not submitted with valid date - redirect
    header("Location: http://".$servername."/".$_SESSION[$ss]['company_code']);
    return;
}
