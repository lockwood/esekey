...
<form action="<?=$_SERVER[PHP_SELF]?>" name="frmAvail" id="frmAvail" method="get">
  ...
  <table width="100%" style="margin-bottom:20px;">
   ...
   <td>
    ...
    <td valign="top" align="right" style="font-size: 12;vertical-align:top;"><?=$property_string?><b>From:</b>
...
<?php
   ...
   <td width="60%" align="right"><?=$bk_string?></td>
  </tr>
 </table>
</form>
<?php
...
while ($month_count <= 3) {
	if ($month_count > 1) {
		// insert column to create gap between months
		?>
<td class="fill">&nbsp;</td>
		<?php
	}
...
<table>
  ...
  while ($datearray[$i]['booking_month'] == $c_month) { ?>
    <td class="calmonth">
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
        <tr><td colspan="7" class="apartment"><?=strtoupper($c_name)?> - <?=strtoupper($montharray[(0 + $c_month)])?> <?=$disp_year?></td></tr>
        <tr>
  ...
  <?php 
    if ($c_name != $fullname) {
    	$c_name = $fullname;
    	if ($datearray[$i]['booking_month'] == $c_month) {
    	?>
    </tr>
    <tr class="calmonth">
    	<?php
