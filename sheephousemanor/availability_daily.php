...
if ($total_count < ($property_count*92)) { // need to add some dates
    ...
    foreach ($resourcearray as $propertyrow) {    	// pre-book b&b rooms so they are unavailable by default and can only be made available via admin console Manage B&B option    	if ($propertyrow['property_id'] == 7 || $propertyrow['property_id'] == 8) {    		$bkref = 2;    	} elseif($propertyrow['property_id'] == 6) {    		$bkref = 1;    	} else {    		$bkref = $booking_reference;    	}
