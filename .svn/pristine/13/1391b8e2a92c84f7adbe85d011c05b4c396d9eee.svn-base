...
<?php
if ($success) { ?>
<body bgcolor="#FFFFFF" text="#000000" link="#0000FF" vlink="#0000FF" alink="#FF0000" onload="LoadEditor(); opener.location.reload(); alert('Record Successfully Updated<?=$msgtext?>')" onunload="opener.location.reload()">
<?php
} else { ?>
<body bgcolor="#FFFFFF" text="#000000" link="#0000FF" vlink="#0000FF" alink="#FF0000" onload="LoadEditor();" onunload="opener.location.reload()">
<?php
} ?>

<!-- Set Workarea -->
<div class="workarea">
<form action="editelement.php" name="frmEdit" id="frmEdit" method="post">
		...
		<table width="100%" border="0" cellspacing="1" cellpadding="3">
                ...
                <tr>
                  ...
                  <td width="85%" class="<?=$class?>">
                           ...
                           <input type="text" name="<?=$column[$i]?>" value="<?=$viewrow[$column[$i]]?>" size="50" maxlength="50" class="input">
                            <?php 
                            } elseif ($type[$i] == 'I') { // input integer field ?> 
                           <input type="text" name="<?=$column[$i]?>" value="<?=$viewrow[$column[$i]]?>" size="8" maxlength="8" class="input">
                            <?php 
                            } elseif ($type[$i] == 'D') { // input date field ?> 
                           <input type="text" name="<?=$column[$i]?>" value="<?=$viewrow[$column[$i]]?>" size="10" maxlength="10" class="input">
                            <?php 
                            } elseif ($type[$i] == '4') { // input datetime field ?> 
                           <input type="text" name="<?=$column[$i]?>" value="<?=$viewrow[$column[$i]]?>" size="19" maxlength="19" class="input">
                            <?php 
                            } elseif ($type[$i] == 'E') { // input email field ?> 
                           <input type="text" name="<?=$column[$i]?>" value="<?=$viewrow[$column[$i]]?>" size="50" maxlength="50" class="input">
                            <?php 
                            } elseif ($type[$i] == 'A') { // textarea field ?> 
                           <textarea name="<?=$column[$i]?>" id="<?=$column[$i]?>" cols="100" rows="20" class="input" onKeyDown="ChangeMade();"><?=$viewrow[$column[$i]]?></textarea> 
                            <?php 
                            } else { // checkbox
                                if ($viewrow[$column[$i]] == 'Y') {
                                    echo '<input type="checkbox" name="'.$column[$i].'" value="Y" CHECKED >'; 
                                } else {
                                    echo '<input type="checkbox" name="'.$column[$i].'" value="Y" >'; 
                                } 
                            } 
                        } ?>
                        </td>
	...
	<table width="100%" height="33" border="0" cellspacing="0" cellpadding="0">
		<tr>
			...
			<td height="33" align="right" valign="bottom" nowrap>
				<input type="submit" name="btnUpdate" value="<?=$btnlabel?>" class="button">
			</td>
		</tr>
	</table>
