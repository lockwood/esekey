<?php
// +----------------------------------------------------------------------+
// | IDXS  - the secure EseSite runtime driver - Company 4                |
// +----------------------------------------------------------------------+
...
// +----------------------------------------------------------------------+
//
// $Id: 4/idxs.php,v 1.01 2005/04/11
//
...
$window_type = 'p';
if ($_GET['p'] != 9) { 
    // secure booking  page 9 only - otherwise redirect to home
    echo "Not page 9";
...
if ((!isset($_GET['conditions'])) && (!isset($_GET['d'])) && (!isset($_GET['ymd']))) { // booking conditions not accepted or form not submitted with valid date - redirect
    header("Location: http://".$servername."/".$_SESSION[$ss]['company_code']);
    return;
}
