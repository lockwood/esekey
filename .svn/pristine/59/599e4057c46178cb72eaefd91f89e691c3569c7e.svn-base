	...
	if (isset($r[$propertyrow['property_id']])) {
        ...
        foreach ($datearray as $daterow) {
            if (($daterow['display_status'] != 'E') && (!isset($_SESSION[$ss]['booking_reference']))) { //this date is NOT available 
                $error1.='Sorry, '.ltrim($propertyrow['property_number']." ".$propertyrow['property_name']).' is not available on '.$daterow['booking_day'].'-'.$daterow['booking_month'].'-'.$daterow['booking_year'].'.<br>';
                return;
            }
        }
    }
...
if ((isset($_GET['requestBooking'])) && (!isset($error1))) { // user has confirmed valid booking request
	include('customer_details_validate.php');
    
    
    if ($z == '') {
        $error2.='Enter credit/debit card number.<br>';
        ...
        $z = $z_no_spaces; 
        if (!ereg("[0-9]{16,19}", $z )) {
            $error2.='Enter a valid credit/debit card number.<br>';
        }
    }
    if (($cardarray[$w] == 'Switch') && (!ereg("[0-9]{1,2}", $v ))) {
        $error2.='Enter a valid Switch Issue Number.<br>';
    }
    if (($cardarray[$w] == 'Maestro') && (!ereg("[0-9]{1,2}", $v ))) {
        $error2.='Enter a valid Maestro Issue Number.<br>';
    }
    if ($cv2 == '') {
        $error2.='Enter a security code (the last 3 digits shown in the signature strip on the reverse of the card).<br>';
