...
for ($i = 0; $i < count($column); $i++) {
    $viewrow[$column[$i]] = '';
    if ($unique[$i] == 'Y') {
        // temporary cludge - $i=0 is page_id and $i=1 is sequence, we only want to increment sequence and not page_id!!
    	$viewrow[$column[$i]] = $lastrow[$column[$i]] + $i;
                           ...
                           <?php
                            ...
                            for ($j = 0; $j < count($listarray[$i]); $j++) {
			                    ...
			                    if ($imagesize[0] > $imagesize[1])
			                    {
			                    	...
			                    	{
			                    		$thumb_width = 400;
			                    		$thumb_height = round(400*$imagesize[1]/$imagesize[0]);
			                    	} else
			                    	{
			                    ...
			                    } else
			                    {
			                    	if ($imagesize[1] > 400)
			                    	{
			                    		$thumb_width = round(400*$imagesize[0]/$imagesize[1]);
			                    		$thumb_height = 400;
			                    	} else
			                    	{
			                    		$thumb_width = $imagesize[0];
                            	...
                            	if ($listarray[$i][$j] == $viewrow[$column[$i]]) { 
                              	$imagedivs .= '<div id="'.$listarray[$i][$j].'"><img src="../'.$_SESSION[$ss]['company_code'].'/'.$listarray[$i][$j].'" width="'.$thumb_width.'" height="'.$thumb_height.'"></div>';
                              	?>
                           <OPTION VALUE="<?=$listarray[$i][$j]?>" SELECTED><?=$listarray[$i][$j]?></OPTION><?php
                              } else { 
                              	$imagedivs .= '<div id="'.$listarray[$i][$j].'" style="display:none;"><img src="../'.$_SESSION[$ss]['company_code'].'/'.$listarray[$i][$j].'" width="'.$thumb_width.'" height="'.$thumb_height.'"></div>';
                              	?>
