		...
		<table width="100%" border="0" cellspacing="1" cellpadding="3">
                ...
                <tr>
                  ...
                  <td width="85%" class="<?=$class?>">
                           ...
                           <?php
                            } 							if (isset($listarray[$i])) {                            	for ($j = 0; $j < count($listarray[$i]); $j++) {    	                          if ($listarray[$i][$j] == $viewrow[$column[$i]]) { ?>
                           <OPTION VALUE="<?=$listarray[$i][$j]?>" SELECTED><?=$listarray[$i][$j].(isset($list_desc_array[$i][$j]) ? $list_desc_array[$i][$j] : '')?></OPTION><?php
        	                      } else { ?>
                           <OPTION VALUE="<?=$listarray[$i][$j]?>"><?=$listarray[$i][$j].(isset($list_desc_array[$i][$j]) ? $list_desc_array[$i][$j] : '')?></OPTION><?php
            	                  }	                            }
                            } ?>
                           </SELECT>
                           <?php
